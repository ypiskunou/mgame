...
import mmf.piskunou.mastermind.*;
...
public class MastermindHandler {
    ...
    private Game buildGameFromSessionAndRequest(HttpServletRequest request) {
        ...
        if (row >= 0) {
            int full = Integer.parseInt(params.get(html.paramNameFull(row)));
            int partial = Integer.parseInt(params.get(html.paramNamePartial(row)));
